...
const COMMANDS = {
  ...
  'agent teardown': { options: { agent:{type:'string'}, 'name-re':{type:'string'}, 'skip-re':{type:'string'}, 'multi-agent':{type:'boolean'}, 'access-points':{type:'boolean'}, 'include-connector':{type:'boolean'}, 'include-marketplace':{type:'boolean'} }, module: 'agent', task: 'W1-H', needsAws: true, dryRunDefault: true, summary: 'delete one agent\'s runtimes and recreatable table items (guarded; --access-points and --include-connector opt in)' },

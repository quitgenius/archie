...
export function createCronTool(deps) {
  ...
  return {
    ...
    description:
      ...
      + '            A run has UP TO 8 HOURS to complete. There is nothing to configure and no\n'
      + '            timeout to set; long jobs are expected and fine.\n'
      + '            IF A RUN IS STILL GOING WHEN THE NEXT FIRE IS DUE, THAT FIRE IS DISCARDED —\n'
      + '            not queued, not delayed, just lost. So pick a period comfortably longer than\n'
      + '            the work takes, or split the job. A 2-hourly job that takes 3 hours silently\n'
      + '            runs half as often as it looks like it does.\n'
